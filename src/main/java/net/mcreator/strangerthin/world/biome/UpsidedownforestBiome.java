...
package net.mcreator.strangerthin.world.biome;
...
import net.mcreator.strangerthin.block.UpsidedownstoneBlock;
import net.mcreator.strangerthin.block.UpsidedownforestdirtBlock;
import net.mcreator.strangerthin.block.UpsideDownForestLogBlock;
...
public class UpsidedownforestBiome extends StrangerthinModElements.ModElement {
	public static Biome biome;

	public UpsidedownforestBiome(StrangerthinModElements instance) {
		super(instance, 15);
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(-6710887).setWaterColor(-14329397).setWaterFogColor(-14329397)
						.withSkyColor(-6710887).withFoliageColor(-13261999).withGrassColor(-13261999).build();
				BiomeGenerationSettings.Builder biomeGenerationSettings = new BiomeGenerationSettings.Builder().withSurfaceBuilder(
						SurfaceBuilder.DEFAULT.func_242929_a(new SurfaceBuilderConfig(UpsidedownforestdirtBlock.block.getDefaultState(),
								UpsidedownstoneBlock.block.getDefaultState(), UpsidedownstoneBlock.block.getDefaultState())));
				biomeGenerationSettings.withFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Feature.TREE
						.withConfiguration(
								(new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(UpsideDownForestLogBlock.block.getDefaultState()),
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.NONE).depth(0.1f).scale(0.15f)
						.temperature(0.5f).downfall(0.5f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("strangerthin:upsidedownforest"));
			}
		}
	}
	...
	public void init(FMLCommonSetupEvent event) {
		BiomeManager.addBiome(BiomeManager.BiomeType.WARM,
				new BiomeManager.BiomeEntry(RegistryKey.getOrCreateKey(Registry.BIOME_KEY, WorldGenRegistries.BIOME.getKey(biome)), 150));
	}
	...
	private static class CustomLeaveVineTreeDecorator extends LeaveVineTreeDecorator {
		...
		static {
			...
			tdt.setRegistryName("upsidedownforest_lvtd");
	...
	private static class CustomTrunkVineTreeDecorator extends TrunkVineTreeDecorator {
		...
		static {
			codec = com.mojang.serialization.Codec.unit(() -> instance);
			tdt = new TreeDecoratorType(codec);
			tdt.setRegistryName("upsidedownforest_tvtd");
			ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
		}
	...
	private static class CustomCocoaTreeDecorator extends CocoaTreeDecorator {
		...
		static {
			codec = com.mojang.serialization.Codec.unit(() -> instance);
			tdt = new TreeDecoratorType(codec);
			tdt.setRegistryName("upsidedownforest_ctd");
			ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
		}
